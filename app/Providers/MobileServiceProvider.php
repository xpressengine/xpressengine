...
class MobileServiceProvider extends ServiceProvider
{
    ...
    {
        Blade::directive('mobileonly', function($expression) {
            return "<?php if(app('request')->isMobile()) { ?>";
        });
        Blade::directive('endmobileonly', function($expression) {
            return "<?php } ?>";
        });
        Blade::directive('desktoponly', function($expression) {
            return "<?php if(!app('request')->isMobile()) { ?>";
        });
        Blade::directive('enddesktoponly', function($expression) {
            return "<?php } ?>";
        });
    }
    ...
    protected function registerRequestRebindHandler()
    {
        $this->app->rebinding(
            ...
            function ($app, $request) {
                $request->setMobileResolver(
                    function (Request $request, $byUserAgent = false) use ($app) {
